...
from example.exampleImpl import example
from example.exampleServer import MethodContext
from example.authclient import KBaseAuth as _KBaseAuth
...
class exampleTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for nameval in config.items('example'):
        ...
        cls.serviceImpl = example(cls.cfg)
    ...
    # NOTE: According to Python unittest naming rules test method names should start from 'test'. # noqa
    def test_run_example_ok(self):
        # call your implementation
        ret = self.serviceImpl.run_example(self.ctx,
    ...
    def test_run_example_min_len_negative(self):
        with self.assertRaisesRegex(ValueError, 'min_length parameter cannot be negative'):
            self.serviceImpl.run_example(self.ctx,
                                              {'workspace_name': self.wsName,
                                               'assembly_input_ref': '1/fake/3',
                                               'min_length': '-10'})

    def test_run_example_min_len_parse(self):
